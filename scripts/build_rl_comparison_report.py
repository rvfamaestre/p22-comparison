...
def coerce_bool(value: object) -> bool:
    ...
    if isinstance(value, (int, float)):
        return not math.isnan(float(value)) and bool(value)
    return str(value).strip().lower() in {"1", "true", "yes", "y"}


def coerce_float(value: object, default: float = float("nan")) -> float:
    try:
        return float(value)
...
def parse_collision_clamp_events(metadata: dict) -> list[dict[str, float | int]]:
    raw_events = metadata.get("collision_clamp_events", [])
    ...
    for event in raw_events:
        if isinstance(event, dict):
            time_value = coerce_float(event.get("time"))
            vehicle_id = int(event.get("vehicle_id", -1))
        elif isinstance(event, (list, tuple)) and len(event) >= 2:
            time_value = coerce_float(event[0])
            vehicle_id = int(event[1])
        else:
            continue

        if math.isfinite(time_value):
            parsed.append({"time": time_value, "vehicle_id": vehicle_id})
    return parsed
...
def compute_eval_metrics(micro_csv_path: Path, metadata_path: Path, config_path: Path | None = None) -> dict[str, float]:
    ...
    initial_conditions = str(
        cfg.get(
            "initial_conditions",
            metadata.get("initial_conditions", "unknown"),
        )
    ).lower()
    warmup_duration = float(
        cfg.get(
            "warmup_duration",
            metadata.get("warmup_duration", 10.0),
        )
    )
    noise_warmup_time = float(
        cfg.get(
            "noise_warmup_time",
            metadata.get("noise_warmup_time", 0.0),
        )
    )
    ...
    analysis_perturbation_time = float(
        metadata.get("perturbation_applied_time", perturbation_time)
    )
    protocol_ready_time = max(warmup_duration, noise_warmup_time)
    protocol_recommended_perturbation_time = (
        protocol_ready_time
        + STRING_STABILITY_BASELINE_WINDOW_STEPS * dt
    )
    perturbation_applied = metadata.get("perturbation_applied")
    if perturbation_applied is None and perturbation_enabled:
        # Historical evaluation artifacts were generated by a simulator that only
        # injected perturbations under uniform initialization.
        perturbation_applied = initial_conditions == "uniform"
    perturbation_applied = coerce_bool(perturbation_applied)
    clamp_events = parse_collision_clamp_events(metadata)
    pre_clamp_count = metadata.get("pre_perturbation_collision_clamp_count")
    post_clamp_count = metadata.get("post_perturbation_collision_clamp_count")
    if pre_clamp_count is None and clamp_events:
        pre_clamp_count = sum(
            1 for event in clamp_events
            if float(event["time"]) < analysis_perturbation_time
        )
    if post_clamp_count is None and clamp_events:
        post_clamp_count = sum(
            1 for event in clamp_events
            if float(event["time"]) >= analysis_perturbation_time
        )

    metrics["warmup_duration"] = warmup_duration
    metrics["noise_warmup_time"] = noise_warmup_time
    metrics["perturbation_time"] = perturbation_time
    metrics["analysis_perturbation_time"] = analysis_perturbation_time
    metrics["protocol_ready_time"] = protocol_ready_time
    metrics["protocol_recommended_perturbation_time"] = protocol_recommended_perturbation_time
    metrics["perturbation_applied"] = perturbation_applied
    metrics["protocol_perturbation_after_warmup"] = analysis_perturbation_time > protocol_ready_time
    metrics["protocol_baseline_window_after_warmup"] = (
        analysis_perturbation_time >= protocol_recommended_perturbation_time
    )
    metrics["pre_perturbation_collision_clamp_count"] = (
        float(pre_clamp_count)
        if pre_clamp_count is not None
        else float("nan")
    )
    metrics["post_perturbation_collision_clamp_count"] = (
        float(post_clamp_count)
        if post_clamp_count is not None
        else float("nan")
    )

    if perturbation_enabled and perturbation_applied and "gap_s" in df.columns:
    ...
    elif perturbation_enabled and not perturbation_applied:
        metrics["string_stability_failure_reason"] = "perturbation_not_applied"
    ...
    if coerce_bool(metrics.get("primary_objective_eligible", False)):
        metrics["admissibility_failure_reason"] = ""
    elif not coerce_bool(metrics.get("perturbation_applied", False)):
        metrics["admissibility_failure_reason"] = "perturbation_not_applied"
    elif not coerce_bool(metrics.get("protocol_baseline_window_after_warmup", False)):
        metrics["admissibility_failure_reason"] = "perturbation_before_post_warmup_baseline"
    elif math.isfinite(coerce_float(metrics.get("pre_perturbation_collision_clamp_count"))) and coerce_float(metrics.get("pre_perturbation_collision_clamp_count")) > 0:
        metrics["admissibility_failure_reason"] = "pre_perturbation_collision_clamp"
    elif not coerce_bool(metrics.get("safety_constraint_satisfied", False)):
        metrics["admissibility_failure_reason"] = "safety_constraint_failed"
    else:
        metrics["admissibility_failure_reason"] = str(
            metrics.get("string_stability_failure_reason", "primary_objective_unavailable")
        )
...
def best_method_for_metric(eval_df: pd.DataFrame, metric: str, human_ratio: float) -> str:
    ...
    if subset.empty:
        return "no admissible winner"
    if "primary_objective_eligible" in subset.columns:
        subset = subset[
            subset["primary_objective_eligible"].map(coerce_bool)
        ]
    if subset.empty:
        return "no admissible winner"
...
def write_markdown_report(
    ...
    lines.append("")
    eligible_mask = (
        eval_df["primary_objective_eligible"].map(coerce_bool)
        if "primary_objective_eligible" in eval_df.columns
        else pd.Series(False, index=eval_df.index)
    )
    lines.append("## Admissibility Gate")
    lines.append("")
    if not bool(eligible_mask.any()):
        lines.append("No admissible winner emerged from this run.")
        lines.append("")
    protocol_invalid = (
        (~eval_df["protocol_baseline_window_after_warmup"].map(coerce_bool)).sum()
        if "protocol_baseline_window_after_warmup" in eval_df.columns
        else 0
    )
    if protocol_invalid:
        lines.append(
            f"- `{protocol_invalid}` / `{len(eval_df)}` evaluations scheduled the perturbation before a full post-warm-up baseline window."
        )
    if "perturbation_applied" in eval_df.columns:
        not_applied = int((~eval_df["perturbation_applied"].map(coerce_bool)).sum())
        if not_applied:
            lines.append(
                f"- `{not_applied}` evaluations did not apply the configured perturbation."
            )
    if "pre_perturbation_collision_clamp_count" in eval_df.columns:
        known_pre = eval_df["pre_perturbation_collision_clamp_count"].dropna()
        if not known_pre.empty:
            pre_invalid = int((known_pre > 0).sum())
            if pre_invalid:
                lines.append(
                    f"- `{pre_invalid}` evaluations recorded pre-perturbation collision clamps."
                )
    lines.append("")
    ...
    lines.append("")
    lines.append("| Method | Tag | HR | Mean Spd | Mean Spd L100 | Spd Var | Spd Var L100 | RMS Acc | RMS Jerk | Min Gap | Collisions | Clamps | Pre-Clamps | Injected | Amp. Ratio | Safety | Eligible | Failure |")
    lines.append("| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |")
    for _, row in eval_df.iterrows():
        ...
        pre_clamps = (
            str(int(float(row["pre_perturbation_collision_clamp_count"])))
            if pd.notnull(row.get("pre_perturbation_collision_clamp_count"))
            else "n/a"
        )
        injected = str(coerce_bool(row.get("perturbation_applied", False)))
        eligible = str(coerce_bool(row.get("primary_objective_eligible", False)))
        lines.append(
            "| "
            + " | ".join(
                [
                    ...
                    safety,
                    eligible,
                    str(row.get("admissibility_failure_reason", "")),
                ]
            )
            + " |"
        )
    ...
    lines.append("")
    lines.append("## Best Admissible Method By Metric And Human Ratio")
    lines.append("")
    for human_ratio in sorted(eval_df["human_ratio"].unique()):
        ...
        subset = eval_df[eval_df["human_ratio"] == human_ratio].copy()
        if "primary_objective_eligible" in subset.columns:
            subset = subset[subset["primary_objective_eligible"].map(coerce_bool)]
        if subset.empty:
            lines.append("- No admissible winner.")
            lines.append("")
            continue
