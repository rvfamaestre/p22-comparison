"""Evaluate a GPU PPO checkpoint across the formal generalization grid.

Usage:
    python evaluate_gpu_ppo.py --config config/rl_train.yaml \
        --checkpoint output/hpc_gpu_train/gpu_ppo_final.pt \
        --device cuda
"""
...
from train_gpu_ppo import build_initial_obs, build_vec_env_config, load_config
...
def build_trainer(
    ...
    trainer = GPUPPOTrainer(
        ...
        minibatch_size=max(1, int(rl.get("minibatch_size", 64))),
...
def select_rl_actions(
    ...
    """Evaluate the PPO policy deterministically for one batched observation."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate a GPU PPO checkpoint on the vectorized ring-road environment."
    )
...
def main() -> None:
    ...
    print(
        f"[evaluate_gpu_ppo] Wrote {len(results['raw_rows'])} raw rows, "
