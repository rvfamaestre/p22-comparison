...
from src.utils.logger import Logger, NullLogger
...
class ScenarioManager:
    ...
    def create_vehicles(self):
        ...
        if nominal_spacing < (min_safe_spacing + vehicle_length):
            ...
            print(
                f"  Recommendation: N <= {int(L / (min_safe_spacing + vehicle_length))} vehicles"
            )
            print(
                f"                  OR L >= {int(N * (min_safe_spacing + vehicle_length))}m"
            )
        ...
        if init_mode == "uniform":
            ...
            print(
                f"  - Spacing: {uniform_spacing:.2f}m (identical for all {N} vehicles)"
            )
            ...
            if self.config.get("perturbation_enabled", False):
                print(
                    f"  - Perturbation: Vehicle {self.config.get('perturbation_vehicle', 0)} at t={self.config.get('perturbation_time', 10.0):.1f}s"
                )
            ...
            min_spacing = 2.5 * max(
                self.config["idm_params"]["s0"], self.config["cth_params"]["d0"]
            )
        ...
        # ---- Determine which ring positions are CAV vs HDV ----
        # By default, slots 0..num_human-1 are HDV, num_human..N-1 are CAV.
        # With shuffle_cav_positions=True, CAVs are randomly interspersed.
        shuffle_cav_positions = self.config.get("shuffle_cav_positions")
        if shuffle_cav_positions is None:
            shuffle_cav_positions = self.config.get("rl", {}).get(
                "shuffle_cav_positions", False
            )

        slot_is_cav = np.array([False] * num_human + [True] * num_cav)
        if shuffle_cav_positions:
            np.random.shuffle(slot_is_cav)
            cav_positions = [i for i, is_cav in enumerate(slot_is_cav) if is_cav]
            print(f"[ScenarioManager] CAV positions SHUFFLED: {cav_positions}")

        vehicles = [None] * N
        hdv_idx = 0  # counter for human sub-type assignment

        # ---------------------------------------------------------------------
        # DUAL HUMAN MODEL: Check if new parameters exist
        # ---------------------------------------------------------------------
        use_dual_model = (
            "stochastic_idm_params" in self.config
            and "unstable_idm_params" in self.config
            and "unstable_human_ratio" in self.config
        )

        # Pre-build human sub-type list
        if use_dual_model:
            ...
            human_subtypes = ["stochastic"] * num_stochastic + [
                "unstable"
            ] * num_unstable
            np.random.shuffle(human_subtypes)
        else:
            print(f"[ScenarioManager] Single Human Model (Legacy)")
            human_subtypes = ["standard"] * num_human

        # ---- Create all vehicles, placing CAVs/HDVs according to slot_is_cav ----
        hdv_counter = 0
        for slot in range(N):
            vid = slot
            x0 = float(x_positions[slot])
            v0 = float(v_perturb[slot])

            if slot_is_cav[slot]:
                # CAV at this ring position
                vehicles[slot] = CAVVehicle(
                    vid=vid,
                    x0=x0,
                    v0=v0,
                ...
                # Human at this ring position
                htype = human_subtypes[hdv_counter]
                hdv_counter += 1
                if htype == "stochastic":
                    vehicles[slot] = StochasticHumanVehicle(
                        vid=vid,
                        x0=x0,
                        v0=v0,
                    ...
                    vehicles[slot] = UnstableHumanVehicle(
                        vid=vid,
                        x0=x0,
                        v0=v0,
    ...
    def build(self, live_viz=None):
        ...
        compute_macro_fields = self.config.get("compute_macro_fields", True)
        if compute_macro_fields:
            macro_gen = MacrofieldGenerator(
                L=self.config["L"], dx=self.config["dx"], h=self.config["kernel_h"]
            )
        else:
            macro_gen = None
        ...
        metadata = {
            ...
            "viz_mode": self.config.get("viz_mode", None),
            ...
            "perturbation_time": self.config.get("perturbation_time", 10.0),
            "perturbation_delta_v": self.config.get("perturbation_delta_v", -2.0),
            "shuffle_cav_positions": self.config.get(
                "shuffle_cav_positions",
                self.config.get("rl", {}).get("shuffle_cav_positions", False),
            ),
            "mesoscopic_enabled": self.config.get("mesoscopic", {}).get(
                "enabled", False
            ),
            "rl_mode": self.config.get("rl", {}).get("rl_mode", "off"),
        ...
        if self.config.get("logging_enabled", True):
            logger = Logger(output_dir=self.config["output_path"], metadata=metadata)
        else:
            logger = NullLogger(
